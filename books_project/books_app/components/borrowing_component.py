...
class BorrowingComponent:
    ...
    def create_borrow(self, borrow_date, book_id, member_id):
        ...
        borrowing_date = datetime.strptime(borrow_date, '%Y-%m-%d').date()
    ...
    def update_borrow(self, borrowing_id, return_date):
        date = datetime.strptime(return_date, '%Y-%m-%d').date()  # Converts string to date object
        borrow_data = {
        'id': borrowing_id,
        'return_date': date,
        }
        ...
        borrow_instance = self.get_borrow(borrowing_id)
        self.fine_component.create_fine(borrow_instance)

    def get_borrow(self, borrowing_id):
        borrowing = BorrowingRepository.get_borrow(borrowing_id)
